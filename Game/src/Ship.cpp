...
Ship::Ship(IComposite* scene, IShapeSFML* background) :
	...
	, m_phisics(new MovementInSpace(1000, 400, 200))
, m_invisibility(2.5)
...
void Ship::Update(const float& deltatime)
{
	...
	m_background->setPosition(static_cast<MovementInSpace*>(m_phisics)->calculPosition(m_background, m_scene->getRoot()->getScene(), m_scene->getRoot()->getScene()->getRefreshTime().asSeconds()));
